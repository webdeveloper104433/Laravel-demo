...
@section('content')
<div style="background-color: @if(array_key_exists('color', $data)) {{ $data['color'] }} @else gray @endif">
      ...
      @if (array_key_exists('google_images', $data))
        @foreach ($data['google_images'] as $google_image)
          <div style="padding-top: 20px; @if (!(array_key_exists('title', $data) && $data['title'] == 'off')) height: 90vh; @else height: 100vh; @endif" class="item @if ($loop->index == 0) active @endif">
              <!-- <div> -->
                <img src="{{ $google_image->url }}" style="max-height: 100%; border: @if (array_key_exists('border', $data)) {{ $data['border'] }}px @else 2px @endif solid #544949; margin-top: -5px;" >
            <!-- </div> -->
          </div>
        @endforeach
      @endif
