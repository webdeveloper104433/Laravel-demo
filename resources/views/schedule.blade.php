          ...
          @foreach ($schedules as $schedule)
            ...
            @endif
                <td class="schedule-border @if (($loop->index + 2) % 2 == 0) left-background-color @else right-background-color @endif">
                  <div>
                    <img src="{{ asset('storage') . '/' . $schedule->image->url }}" class="img-thumbnail @if (($schedules->count() == $loop->index + 1) && (($loop->index + 1) % 2 == 1) && (($loop->index + 1) % 4 == 1)) image-one-item  @elseif ((($schedules->count() == $loop->index + 2) && (($loop->index + 2) % 4 == 2)) || (($schedules->count() == $loop->index + 1) && (($loop->index + 1) % 4 == 2))) image-one-row  @else image-item @endif">
                  </div>
                  <div style="">
                    <strong>
